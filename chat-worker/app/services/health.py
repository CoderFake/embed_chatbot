...
async def get_detailed_health() -> DetailedHealthResponse:
    ...
    try:
        await service_manager.redis.ping()
        components["redis"] = "healthy"
    except Exception as e:
        components["redis"] = f"unhealthy: {str(e)}"
    
    try:
        async with service_manager.get_db_session() as session:
    ...
    try:
        if service_manager.milvus_client:
            components["milvus"] = "healthy"
        else:
            components["milvus"] = "not initialized"
    except Exception as e:
        components["milvus"] = f"error: {str(e)}"
    
    try:
        components["queue_consumer"] = "running" if chat_queue_consumer._consuming else "stopped"
    except Exception as e:
